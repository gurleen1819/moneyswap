...
export default function HomeScreen({ navigation }) {
 ...
 useEffect(() => {
  fetchRate();
}, [fromCurrency, toCurrency]); // track both
  ...
  return (
    <View style={styles.container}>
      ...
      <CurrencyDropdown
        label="From"
        value={fromCurrency}
        onPress={() => navigation.navigate("CurrencySelection", { onSelect: setFromCurrency })}
      />
      <CurrencyDropdown
        label="To"
        value={toCurrency}
        onPress={() => navigation.navigate("CurrencySelection", { onSelect: setToCurrency })}
      />
    <Button title="Convert" onPress={handleConvert} />

      {converted ? <Text style={styles.result}>Converted: {toCurrency} {converted}</Text> : null}
